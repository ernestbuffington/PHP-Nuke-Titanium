...
function modadmin_dispaly_modules($modadmin_modules) 
{
   ...
   foreach ($modadmin_modules as $module) 
   {
      ...
      {
         $home       = get_evo_icon('evo-sprite home tooltip', $admlang['modules']['inhome']);
         $active     = get_evo_icon('evo-sprite ok tooltip', $admlang['global']['active']);
         $title      = "<strong>".$module['title']."</strong>";
         $who_view   = "<strong>".$who_view."</strong>";
         // $bgcolor = ' bgcolor="'.$bgcolor4.'"';
      } else {
         $home       = '<a href="'.$admin_file.'.php?op=modules&amp;h='.$module['mid'].'">'.get_evo_icon('evo-sprite cancel tooltip', $admlang['global']['inactive']).'</a>';
         $active     = (intval($module['active'])) ? '<a href="'.$admin_file.'.php?op=modules&amp;a='.$module['mid'].'">'.get_evo_icon('evo-sprite ok tooltip', $admlang['global']['active']).'</a>' : '<a href="'.$admin_file.'.php?op=modules&amp;a='.$module['mid'].'">'.get_evo_icon('evo-sprite cancel tooltip', $admlang['global']['inactive']).'</a>';
      ...
      echo '    <td class="row1"><a href="'.$admin_file.'.php?op=modules&amp;edit='.$module['mid'].'">'.get_evo_icon('evo-sprite edit tooltip').'</a></td>'; // '._EDIT.'
      echo '  </tr>';
   }
   echo '</table>';
...
function modadmin_block () {
    ...
    if(is_array($inactive)) 
    {
        ...
        {
            // $custom_title = (substr($element['title'],0,3) == '~l~') ? "<span style=\"color: blue;\">".substr($element['title'],3)."</span>&nbsp;&nbsp;<a href=\"".$admin_file.".php?op=modules&amp;delete=".$element['mid']."\">".get_evo_icon('evo-sprite delete')."</a>" : $element['custom_title'];
            ...
            echo '<li class="'.(($element['active'] == 1) ? "active" : "inactive").'" id="mod'.$element['mid'].'" ondblclick="change_status('.$element['mid'].')">'
                .'  <table class="col-12">'
                .'    <tr>'
                .'      <td class="col-10">'.$element['custom_title'].'</td>'
                .'      <td class="col-2"><a href="'.$admin_file.'.php?op=modules&amp;edit='.$row['mid'].'">'.get_evo_icon('evo-sprite edit tooltip', $admlang['modblock']['modedit']).'</a></td>'
                .'    </tr>'
                .'  </table>'
                .'</li>';
        }
    ...
    if(is_array($modadmin_module_cats)) 
    {
        ...
        foreach ($modadmin_module_cats as $cat) 
        {
            ...
            {
                $updown = "<a href=\"".$admin_file.".php?op=modules&amp;upcat=".$cat['pos']."\">".get_evo_icon('evo-sprite sort-up tooltip', $admlang['modblock']['sort_up'])."</a>";
            } 
            else if($i != 1) 
            {
                $updown = "<a href=\"".$admin_file.".php?op=modules&amp;downcat=".$cat['pos']."\">".get_evo_icon('evo-sprite sort-down tooltip', $admlang['modblock']['sort_down'])."</a><a href=\"".$admin_file.".php?op=modules&amp;upcat=".$cat['pos']."\">".get_evo_icon('evo-sprite sort-up tooltip', $admlang['modblock']['sort_up'])."</a>";
            } 
            else if($i == 1) 
            {
                $updown = "<a href=\"".$admin_file.".php?op=modules&amp;downcat=".$cat['pos']."\">".get_evo_icon('evo-sprite sort-down tooltip', $admlang['modblock']['sort_down'])."</a>";
            }
            echo "<span style=\"font-weight: bold; text-align: 'center';\">".$cat['name']."&nbsp;&nbsp;<a href=\"".$admin_file.".php?op=modules&amp;editcat=".$cat['cid']."\">".get_evo_icon('evo-sprite edit tooltip', $admlang['modblock']['edit'])."</a>&nbsp;<a href=\"".$admin_file.".php?op=modules&amp;deletecat=".$cat['cid']."\">".get_evo_icon('evo-sprite trash-2 tooltip', $admlang['modblock']['delete'])."</a>&nbsp;".$updown."</span>";
            ...
            while ($row = $db->sql_fetchrow($result)) 
            {
                ...
                // $custom_title .= "&nbsp;&nbsp;<a href=\"".$admin_file.".php?op=modules&amp;edit=".$row['mid']."\">".get_evo_icon('evo-sprite edit tooltip', $admlang['modblock']['modedit'])."</a>";
