 ...
 *****************************************************/
	$strip_var_list = array('email' => 'email', 'bday_day' => 'bday_day', 'bday_month' => 'bday_month', 'bday_year' => 'bday_year', 'facebook' => 'facebook', 'website' => 'website', 'location' => 'location', 'occupation' => 'occupation', 'interests' => 'interests', 'confirm_code' => 'confirm_code', 'glance_show' => 'glance_show');
/*****[END]********************************************
 [ Mod:    Birthdays                           v3.0.0 ]
 [ Mod:    At a Glance Options                 v1.0.0 ]
 *****************************************************/
 ...
 ******************************************************/
			...
			SET " . $username_sql . $passwd_sql . "user_email = '" . str_replace("\'", "''", $email) ."', " . $birthday_sql . "birthday_display = $birthday_display, birthday_greeting = $birthday_greeting, user_facebook = '" . str_replace("\'", "''", str_replace(' ', '+', $facebook)) . "', user_website = '" . str_replace("\'", "''", $website) . "', user_occ = '" . str_replace("\'", "''", $occupation) . "', user_from = '" . str_replace("\'", "''", $location) . "',user_from_flag = '$user_flag', user_interests = '" . str_replace("\'", "''", $interests) . "', user_glance_show = '". intval($glance_show) . "', user_viewemail = '$viewemail', user_attachsig = $attachsig, user_allowsmile = '$allowsmilies', user_showavatars = '$showavatars', user_showsignatures = '$showsignatures', user_allowhtml = '$allowhtml', user_allowbbcode = '$allowbbcode', user_allow_viewonline = '$allowviewonline', user_notify = '$notifyreply', user_notify_pm = '$notifypm', user_allow_mass_pm = '$allow_mass_pm', user_popup_pm = '$popup_pm', user_timezone = '$user_timezone', user_time_mode = $time_mode, user_dst_time_lag = $dst_time_lag, user_dateformat = '" . str_replace("\'", "''", $user_dateformat) . "', user_show_quickreply = '$user_show_quickreply', sceditor_in_source = '$sceditor', user_quickreply_mode = '$user_quickreply_mode', user_wordwrap = '" . str_replace("\'", "''", $user_wordwrap) . "', user_open_quickreply = $user_open_quickreply, user_lang = '" . str_replace("\'", "''", $user_lang) . "', theme = '$user_style', user_active = '$user_active', user_actkey = '" . str_replace("\'", "''", $user_actkey) . "'" . $avatar_sql . ", user_gender = '$gender', name = '".$rname."', newsletter = '".$newsletter."', bio = '".$extra_info."', user_hide_images = '$hide_images'
			WHERE user_id = '$user_id'";
/*****[END]********************************************
 [ Mod:     Birthdays                          v3.0.0 ]
 [ Mod:     Gender                             v1.2.6 ]
...
/*****[BEGIN]******************************************
 ...
 [ Mod:     Birthdays                          v3.0.0 ]
 ******************************************************/
			$sql = "INSERT INTO " . USERS_TABLE . " (user_id, username, user_regdate, user_password, user_email, user_birthday, user_birthday2, birthday_display, birthday_greeting, user_website, user_occ, user_from, user_from_flag, user_interests, user_glance_show, user_sig, user_sig_bbcode_uid, user_avatar, user_avatar_type, user_viewemail, user_facebook, user_attachsig, user_allowsmile, user_showavatars, user_showsignatures, user_allowhtml, user_allowbbcode, user_allow_viewonline, user_notify, user_notify_pm, user_allow_mass_pm, user_popup_pm, user_timezone, user_time_mode, user_dst_time_lag, user_dateformat, user_show_quickreply, user_quickreply_mode, user_wordwrap, user_open_quickreply, user_lang, theme, user_level, user_allow_pm, name, newsletter, bio, user_hide_images, user_gender, user_active, user_actkey)
			VALUES ('$user_id', '" . str_replace("\'", "''", $username) . "', '" . $reg_date . "', '" . str_replace("\'", "''", $new_password) . "', '" . str_replace("\'", "''", $email) . "', $user_birthday, $user_birthday2, $birthday_display, $birthday_greeting, '" . str_replace("\'", "''", $website) . "', '" . str_replace("\'", "''", $occupation) . "', '" . str_replace("\'", "''", $location) . "','$user_flag', '" . str_replace("\'", "''", $interests) . "', '" . str_replace("\'", "''", $glance_show) . "', '" . str_replace("\'", "''", $signature) . "', '$signature_bbcode_uid', '$user_avatar', '$user_avatar_type', '$viewemail', '" . str_replace("\'", "''", $facebook) . "', '$attachsig', '$allowsmilies', '$showavatars', '$showsignatures', '$allowhtml', '$allowbbcode', '$allowviewonline', '$notifyreply', '$notifypm', '$allow_mass_pm', '$popup_pm', '$user_timezone', '$time_mode', '$dst_time_lag', '" . str_replace("\'", "''", $user_dateformat) . "', '$user_show_quickreply', '$user_quickreply_mode', '" . str_replace("\'", "''", $user_wordwrap) . "', $user_open_quickreply, '" . str_replace("\'", "''", $user_lang) . "', '$user_style', '1', '1', '".$realname."', '".$newsletter."', '".$extra_info."', '$hide_images', '$gender', ";
/*****[END]********************************************
 [ Mod:     Birthdays                          v3.0.0 ]
 [ Mod:     Gender                             v1.2.6 ]
 [ Mod:     Member Country Flags               v2.0.7 ]
