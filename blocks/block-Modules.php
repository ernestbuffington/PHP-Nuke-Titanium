...
function moduleblock_display_inactive() {
    ...
    $content .= "<select name=\"name1\" onchange=\"top.location.href=this.options[this.selectedIndex].value\">\n";
    ...
    if(is_array($moduleblock_invisible)) {
        foreach ($moduleblock_invisible as $module) {
            if ($module['active']) {
                $one = 1;
                $content .= "<option value=\"modules.php?name=".$module['title']."\">".limit_string($module['custom_title'],13)."</option>\n";
            } else {
