 ...
 ******************************************************/
                ...
                if ( $mode != 'edit' )
                {
                        ...
                        if ( $to_userdata['user_notify_pm'] && !empty($to_userdata['user_email']) && $to_userdata['user_active'] )
                        {
                            ...
                            //     $message_text,
                            //     $server_protocol.$server_name.$server_port.$script_name.'&folder=inbox&mode=read&p='.$privmsg_sent_id,
                            //     (!empty($board_config['board_email_sig'])) ? str_replace('<br />', "\n", "-- \n" . $board_config['board_email_sig']) : ''
                            // );

                            // $phpmailer_parameters = array(
                            //     'from' => $board_config['board_email'],
                            //     'fromname' => $board_config['sitename'],
                            //     'addaddress' => $to_userdata['user_email'],
                            //     'ishtml' => true,
                            //     'subject' => $lang['Notification_subject'],
                            //     'body' => str_replace($notify_body_pattern, $notify_body_replace, $lang['private_message_notify'])
                            // );

                            // evo_phpmailer($phpmailer_parameters);

                            $email_data = array(
                                'email'         => $to_userdata['user_email'],
                                'subject'       => $lang['Notification_subject'],
                                'sitename'      => $board_config['sitename'],
                                'to_username'   => $to_userdata['username'],
                                'from_username' => htmlspecialchars($userdata['username']),
                            ...
                            $content = str_replace( '{USERNAME}', $email_data['to_username'],  $lang['private_message_notify'] );
                            $content = str_replace( '{SENDER_USERNAME}', $email_data['from_username'],  $content );
                            $content = str_replace( '{SITENAME}', $email_data['sitename'],  $content );
                            $content = str_replace( '{PM_MESSAGE}', $email_data['message'],  $content );
                            $content = str_replace( '{U_INBOX}', $email_data['inbox_url'],  $content );
                            $content = str_replace( '{EMAIL_SIG}', $email_data['signature'],  $content );

                            $headers = array(
                                'From: '.$email_data['sitename'].' <'.$board_config['board_email'].'>',
