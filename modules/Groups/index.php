...
function generate_user_info(&$row, $date_format, $group_mod, &$from, &$posts, &$joined, &$poster_avatar, &$profile_img, &$profile, &$search_img, &$search, &$pm_img, &$pm, &$email_img, &$email, &$www_img, &$www, &$userdata, &$online_status_img, &$online_status)
{
    ...
    if (!empty($row['user_viewemail']) || $group_mod) {
        ...
        $email_img = '<a href="' . $email_uri . '"><img src="' . $images['icon_email'] . '" alt="' . sprintf($lang['Send_email'],$row['username']) . '" title="' . sprintf($lang['Send_email'],$row['username']) . '" border="0" /></a>';
    ...
    $pm_img   = '<a href="' . $temp_url . '"><img src="' . $images['icon_pm'] . '" alt="' . sprintf($lang['Send_private_message'],$row['username']) . '" title="' . sprintf($lang['Send_private_message'],$row['username']) . '" border="0" /></a>';
    ...
    if ($row['user_session_time'] >= (time() - $board_config['online_time'])) 
    {
        ...
        {
            $online_status_img = '<a href="' . append_sid("viewonline.$phpEx") . '"><img src="' . $images['icon_online'] . '" alt="' . sprintf($lang['is_online'], $row['username']) . '"  /></a>';
            $online_status     = '<a href="' . append_sid("viewonline.$phpEx") . '"' . $online_color . '>' . $lang['Online'] . '</a>';
        } 
        elseif (!empty($row['user_allow_viewonline']) || $group_mod || $userdata['user_id'] == $row['user_id']) 
        {
            $online_status_img = '<a href="' . append_sid("viewonline.$phpEx") . '"><img src="' . $images['icon_hidden'] . '" alt="' . sprintf($lang['is_hidden'], $row['username']) . '" /></a>';
            $online_status     = '<em><a href="' . append_sid("viewonline.$phpEx") . '" title="' . sprintf($lang['is_hidden'], $row['username']) . '"' . $hidden_color . '>' . $lang['Hidden'] . '</a></em>';
        } 
        else 
        {
            ...
            $online_status     = '<span title="' . sprintf($lang['is_offline'], $row['username']) . '"' . $offline_color . '>' . $lang['Offline'] . '</span>';
        }
    } 
    else 
    {
    ...
    if ($confirm) 
	{
        ...
        ******************************************************/
        ...
        if ($userdata['user_level'] != ADMIN && $userdata['user_level'] == MOD) {
            $sql = "SELECT COUNT(auth_mod) AS is_auth_mod FROM (" . AUTH_ACCESS_TABLE . " aa, " . USER_GROUP_TABLE . " ug) WHERE ug.user_id = " . $userdata['user_id'] . " AND aa.group_id = ug.group_id AND aa.auth_mod = '1'";
            if (!($result = $db->sql_query($sql))) {
...
if ($group_id) 
{
    ...
    $sql = "SELECT g.group_moderator, g.group_type, aa.auth_mod FROM ( " . GROUPS_TABLE . " g LEFT JOIN " . AUTH_ACCESS_TABLE . " aa ON aa.group_id = g.group_id ) WHERE g.group_id = '$group_id'";
    ...
    if ($group_info = $db->sql_fetchrow($result)) 
	{
        ...
        if (!empty($_POST['add']) || !empty($_POST['remove']) || isset($_POST['approve']) || isset($_POST['deny'])) 
        {
            ...
            if (isset($_POST['add'])) 
            {
                ...
                $sql      = "SELECT user_id, user_email, user_lang, user_level FROM " . USERS_TABLE . " WHERE username = '" . str_replace("\'", "''", $username) . "'";
                if (!($result = $db->sql_query($sql))) 
                {
                ...
                $sql = "SELECT ug.user_id, u.user_level FROM (" . USER_GROUP_TABLE . " ug, " . USERS_TABLE . " u) WHERE u.user_id = " . $row['user_id'] . " AND ug.user_id = u.user_id AND ug.group_id = '$group_id'";
                if (!($result = $db->sql_query($sql))) 
                {
                ...
                {                    
                    ...
                    if (isset($_POST['approve'])) 
                    {
                        ...
                        ******************************************************/
                        ...
                        if ($sql) 
                        {                            
                            $sql = "UPDATE " . USERS_TABLE . " SET " . $sql . " WHERE user_id IN ($sql_in)";
                            if (!$db->sql_query($sql)) 
                            {
                    ...
                    else if (isset($_POST['deny']) || isset($_POST['remove'])) 
                    {
                        if ($group_info['auth_mod']) 
                        {
                            ...
                            if ($row = $db->sql_fetchrow($result)) 
                            {
                                ...
                                if ($remove_mod_sql != '') 
                                {
                                    $sql = "UPDATE " . USERS_TABLE . " SET user_level = " . USER . " WHERE user_id IN ($remove_mod_sql) AND user_level NOT IN (" . ADMIN . ")";
                                    if (!$db->sql_query($sql)) 
                                    {
                                        message_die(GENERAL_ERROR, 'Could not update user level', '', __LINE__, __FILE__, $sql);
                                    }
    ...
    [ Mod:    Online/Offline/Hidden               v2.2.7 ]
    ******************************************************/
    $sql = "SELECT username, user_id, user_viewemail, user_posts, user_regdate, user_from, user_website, user_email, user_allow_viewonline, user_session_time
                FROM " . USERS_TABLE . "
    ...
    ******************************************************/
    ...
    [ Mod:    Online/Offline/Hidden               v2.2.7 ]
    ******************************************************/
    $sql = "SELECT u.username, u.user_id, u.user_viewemail, u.user_posts, u.user_regdate, u.user_from, u.user_website, u.user_email, ug.user_pending, u.user_allow_viewonline, u.user_session_time
    ...
    ******************************************************/
    ...
    [ Mod:    Online/Offline/Hidden               v2.2.7 ]
    ******************************************************/
    $sql = "SELECT u.username, u.user_id, u.user_viewemail, u.user_posts, u.user_regdate, u.user_from, u.user_website, u.user_email, u.user_allow_viewonline, u.user_session_time
    ...
    ******************************************************/
    ...
    [ Mod:    Online/Offline/Hidden               v2.2.7 ]
    ******************************************************/
    generate_user_info($group_moderator, $board_config['default_dateformat'], $is_moderator, $from, $posts, $joined, $poster_avatar, $profile_img, $profile, $search_img, $search, $pm_img, $pm, $email_img, $email, $www_img, $www, $userdata, $online_status_img, $online_status);
    /*****[END]********************************************
    [ Mod:    Online/Offline/Hidden               v2.2.7 ]
    ******************************************************/
    ...
    for ($i = $start; $i < min($board_config['topics_per_page'] + $start, $members_count); $i++) {
        ...
        ******************************************************/
        ...
        [ Mod:    Online/Offline/Hidden               v2.2.7 ]
        ******************************************************/
        generate_user_info($group_members[$i], $board_config['default_dateformat'], $is_moderator, $from, $posts, $joined, $poster_avatar, $profile_img, $profile, $search_img, $search, $pm_img, $pm, $email_img, $email, $www_img, $www, $userdata, $online_status_img, $online_status);
        /*****[END]********************************************
        [ Mod:    Online/Offline/Hidden               v2.2.7 ]
        ******************************************************/
    ...
    if ($is_moderator) 
	{
        ...
        if ($modgroup_pending_count) {
            for ($i = 0; $i < $modgroup_pending_count; $i++) {
                ...
                generate_user_info($modgroup_pending_list[$i], $board_config['default_dateformat'], $is_moderator, $from, $posts, $joined, $poster_avatar, $profile_img, $profile, $search_img, $search, $pm_img, $pm, $email_img, $email, $www_img, $www, $userdata, $online_status_img, $online_status);
