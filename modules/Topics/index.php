...
if ($db->sql_numrows($result) > 0) 
{
    ...
    while ($row = $db->sql_fetchrow($result)) 
	{
		...
		$output .= '<legend align="center" id="Legend5" runat="server" visible="true" style="width:auto; margin-bottom: 0px; font-size: 18px; font-weight: bold;"><a href="modules.php?name=News&amp;new_topic="'.$topicid.'">'.$topictext.'</a></legend>';
		$output .= '<table border="1" width="100%" align="center" cellpadding="2">';
        
        $output .= '<tr>';
        $output .= '<td valign="top" width="35%"><a href="modules.php?name=News&amp;new_topic="'.$topicid.'"><img src='.$t_image.' border="0" alt='.$topictext.' title='.$topictext.' hspace="5" vspace="5"></a><br /><br />';
        $output .= '<span class="content">';
        $output .= '<img class="icons" align="absmiddle" width="16" src="'.img('topic-16.png','Topics').'"> <strong>'._TOPIC.' :</strong><a href="modules.php?name=News&amp;new_topic="'.$topicid.'"><strong> '.$topictext.'</strong></a><br />';
        $output .= '<img class="icons" align="absmiddle" width="16" src="'.img('topic-blogs-16.png','Topics').'"> <strong>'._TOTNEWS.' </strong>( <font color="'.$digits_color.'"><strong>'.$row['stories'].'</strong></font> )<br />';
        $output .= '<img class="icons" align="absmiddle" width="16" src="'.img('reads-icon-16.png','Topics').'"> <strong>'._TOTREADS.' </strong>( <font color="'.$digits_color.'"><strong>'.(isset($row['readcount']) ? $row['readcount'] : 0).'</strong></font> )</span>';
        ...
        if ($row['stories'] > 0) 
		{
		    ...
		    while ($row2 = $db->sql_fetchrow($result2)) 
			{
                ...
                echo '<img class="icons" align="absmiddle" width="16" src="'.img('topic-blogs-16.png','Topics').'"> '.$cat_link.'<a href="modules.php?name=News&amp;file=article&amp;sid='.intval($row2['sid']).'">'.htmlentities($row2['title']).'</a><br />';
            }
            
			if ($row['stories'] > 0) 
			{
                echo '<div align="right"><a href="modules.php?name=News&amp;new_topic='.$topicid.'"><strong><img align="absmiddle" height="50" src="'.img('more.png','Topics').'"></strong></a></div>';
            }
        } 
		else 
		{
